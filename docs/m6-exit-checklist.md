...
| 升级兼容矩阵 | ✅ 跨版本门禁闭环 | 已自动覆盖完整历史 Alembic revision 升级链、空配置原子首装、迁移失败不切换和 Runtime 启动安全升级；`v0.1.7` Release workflow run `35302608582` 已完成 `v0.1.6 → v0.1.7 → 恢复 v0.1.6` 相邻版本门禁，并验证独立 helper、单容器 transient helper、Compose labels 与 docker.sock 保留；当前 `release-baseline.json` 已推进到正式 `v0.1.7` |
...
| 镜像/SBOM/发布产物 | ✅ 发布闭环 | 当前最新正式 `v0.1.7` Release workflow run `35302608582` 已完成 linux/amd64 GHCR 镜像、SPDX SBOM、release manifest、SHA256SUMS 与 GitHub Release 发布；正式不可变 digest 为 `sha256:d60027f8f72bf3001c1a18b935ab82f6a793ea435d1a3e6482efe6fc5b0ea68a` |
| 运维 runbook/用户手册 | ✅ 闭环 | 已补 Compose 备份/停止/恢复/readiness/回滚步骤；`v0.1.7` 已正式支持显式挂载 docker.sock 的 Compose 单容器通过 Web 一键升级，并由真实 Docker E2E 验证 labels、socket、healthcheck 与失败回滚；无 Docker 管理权限或复杂拓扑时仍保留手工 digest runbook |
