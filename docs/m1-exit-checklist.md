...
- 蜂巢级别真实大包 torrent 已收到并完成脱敏元数据级检查；`runtime/data/downloads` 与 `downloads2` 已在项目容器内完成只读 inventory 扫描，日志与可提交合成等价案例按后续真实测试结果补齐。
- 7 个失败样例暂不预置，后续真实测试遇到问题时逐例归档触发条件、期望结果与人工归因。
- 首批下载器版本已确认为 qBittorrent 5.2.3、Transmission 4.1.3；NAS 文件系统、容器路径和目标目录布局矩阵待真实环境联调时补齐。

HHClub 外部输入已关闭：已确认当前主站为 `https://hhanclub.net`、NexusPHP + Cookie 鉴权、`torrents.php` 搜索与 `details.php` / `download.php` 详情取种链路，并完成真实账号只读登录、搜索和详情验收。
