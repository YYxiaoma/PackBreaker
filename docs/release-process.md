...
当前最新正式版本为 `v0.1.6`。Release workflow run `35297829243` 已成功完成 tag/version、上一正式版 baseline、全量质量门、`v0.1.5 → v0.1.6 → v0.1.5` 真实 Docker 升级/回滚、独立 updater helper 与单容器 transient helper E2E，并发布 linux/amd64 GHCR 镜像、release manifest、SPDX SBOM、`SHA256SUMS` 与 GitHub Release 资产。release 身份绑定 commit `a148aef5c2062829246c8a9f85e76c846da213ed` 和不可变 digest `sha256:b250b4dd945648fca884989d4c6ce14692dea839d4364f462d6080806357f13d`。Release 资产发布成功后 workflow 已推进 `latest`/`stable`；这两个 tag 仍只是可移动发现通道，生产安装、升级、回滚与下一候选的相邻版本门禁继续以 release manifest / `release-baseline.json` 中的完整不可变 digest 为准。
