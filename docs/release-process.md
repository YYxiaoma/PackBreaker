...
当前最新正式版本为 `v0.1.7`。Release workflow run `35302608582` 已成功完成 tag/version、上一正式版 baseline、全量质量门、`v0.1.6 → v0.1.7 → v0.1.6` 真实 Docker 升级/回滚、独立 updater helper 与单容器 transient helper E2E（含 Compose labels 与 docker.sock 保留），并发布 linux/amd64 GHCR 镜像、release manifest、SPDX SBOM、`SHA256SUMS` 与 GitHub Release 资产。release 身份绑定 commit `38f05b4aa91e49ade3ccd7bc033a62bfa8f0df26` 和不可变 digest `sha256:d60027f8f72bf3001c1a18b935ab82f6a793ea435d1a3e6482efe6fc5b0ea68a`。Release 资产发布成功后 workflow 已推进 `latest`/`stable`；这两个 tag 仍只是可移动发现通道，生产安装、升级、回滚与下一候选的相邻版本门禁继续以 release manifest / `release-baseline.json` 中的完整不可变 digest 为准。
