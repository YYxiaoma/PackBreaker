...
99% 修复 planner 仍必须作为纯只读能力测试：v1 mismatch piece 的 `covered_files` 要能识别真实跨文件边界，padding/零长度范围不能制造伪跨文件风险；v2 piece 始终保持文件内作用域。自动 piece 模式在目标仍与源共享 inode、目标 `link_count > 1`、下载器未暂停或隔离/缺失文件预算超过可用空间时不得进入 ready；FILE_ONLY 还必须额外拒绝跨文件 piece 和任何需要隔离的目标。缺失 NFO/图片/字幕只能规划为目标侧完整文件获取，不允许产生源目录写入。`inspect_repair_target` 测试要证明 hardlink/独立 inode/missing target/symlink/source snapshot 漂移均被正确区分且检查前后没有文件副作用。即使受控 repair execute 已开放，所有公开 planner 仍断言 `execution_allowed=false`，证明 GET 快照本身从不构成写授权。

journal-backed inode 隔离原语单独执行写侧故障注入：只有同 task 的 `APPLIED CREATE_HARDLINK` journal 可授权，正常执行后 target 必须变为单链接独立 inode、内容与未修改 source 一致；十路并发只能形成一个 isolation journal 与一次有效替换。临时 inode 创建后但 ownership progress 提交前崩溃必须留下 `INTENT_RECORDED`，重试不得按文件名认领临时资源，而要稳定失败并转 `RECONCILE_REQUIRED`；progress 提交后崩溃允许凭 inode 身份续跑；atomic replace 后响应丢失允许在 target inode 与已登记临时 inode一致、临时路径消失且 source/target 字节相等时恢复 APPLIED。外部替换 target 必须原样保留，isolation operation 也不得被通用 filesystem rollback 当成可删除资源。

task-level isolation coordinator 还要覆盖 ownership handoff 与逐项重规划：每次只执行当前可信 AUTO_PIECE plan 的一个 isolation target，完成后必须重新读取 downloader stopped/ownership、source inventory、execution plan 和 filesystem journal，再决定下一项；完成的 APPLIED isolation 必须成为 repair-plan 当前 target 的 ownership 证据并把 isolation budget 降为 0，原 hardlink journal 保持 APPLIED 不改历史。`INTENT_RECORDED` isolation 必须优先按原 journal 恢复，多个并行未决 isolation 或 `RECONCILE_REQUIRED` 不得自动选择。RETRY + operation journal 的取消必须进入资源取消链。若要求资源回滚，测试必须证明 downloader remove 先完成，再以独立 `CLEANUP_REPAIR_TARGET` journal 删除仍匹配 isolation device/inode/size、`link_count=1` 且 `user.packbreaker.repair_owner` 与 isolation journal UUID 一致的 target；客户端下载原地改写内容/mtime必须保留 ownership marker，而 marker 缺失/篡改、目标文件系统不支持 `user.*` xattr、同尺寸外部替换（包括 inode 被内核立即复用）都必须保留现场并转 `RECONCILE_REQUIRED`。unlink 后响应丢失应从路径不存在恢复同一 cleanup journal，取消重启不得二次 remove；不要求资源回滚时 target 必须保留同一独立 inode，并在 v3 cancellation checkpoint 中显式登记 retained isolation。整个流程 source inode、size、mtime 与内容必须不变，历史 hardlink/isolation journal 也不得被改写为 rolled back。

downloader repair executor 必须覆盖完整 `RETRY → isolation → CLIENT_VERIFYING repair download → stop → 独立二次 recheck/verify → SEEDING/DONE` 链。qB/TR repair start/stop 使用与做种 start 分离的 operation type 和幂等键；start 只能绑定同 plan 的 APPLIED add、上一轮 APPLIED verify/recheck、repair checkpoint 与全部 APPLIED isolation，stop 只能绑定同轮 APPLIED repair-start 并由真实状态证明 100%。响应丢失只能通过 owned torrent 状态恢复，绝不盲目重复 start/stop。四类 repair start/stop operation 的维护对账还必须证明“有 after snapshot 才能自动重新证明”：qB/TR 均只调用 get/read 接口，复核原 downloader version、hash、save path、ownership 和不倒退的 progress；STOP 必须仍为 stopped+complete，START 可保持活动或已经安全推进到 stopped+complete。缺 after snapshot、checking、进度倒退、所有权/绑定漂移均不得标为可安全 reconcile，下载器写调用次数必须保持 0。第二轮校验必须使用独立 `repair_candidate_key`，不得重放首次 verify journal。若二次校验仍不完整，下一轮允许复用客户端下载已写过的 target，但必须证明 source snapshot 未变、target 仍是 isolation journal 登记的同一 device+inode、`link_count=1`、长度正确且与 source inode 不同；target inode 替换必须阻断。测试同时断言整个链路 source 内容、inode、size、mtime 不变。

公开 repair execute API 还必须覆盖独立 action receipt 边界：admin session 缺 CSRF 必须在 service 前阻断；缺失/非法 `Idempotency-Key` 分别稳定返回 428/422，相同 actor+key 的请求只能命中同一 receipt。故障注入至少覆盖 coordinator 已完成 inode isolation 与 repair checkpoint 推进、但 receipt 尚未 SUCCEEDED 时响应丢失，重放必须凭当前服务端 checkpoint 补记同一 receipt 且不新增 isolation。请求体只能有固定 execute 动作，响应/receipt payload 不得出现 torrent hash、ownership、operation journal、repair candidate/evidence digest、device/inode 或绝对路径。

可信 repair-plan API 额外要求“服务端证明、客户端零安全事实输入”：只有 `RETRY + CLIENT_VERIFICATION_INCOMPLETE` checkpoint，且 latest ready plan、ADD/VERIFY APPLIED journal、downloader version/binding/save path、hash+ownership、当前 stopped 状态全部一致时才允许进入 target hash；未暂停必须在任何 target piece 读取前阻断。HARDLINK 文件必须由匹配 source path/source snapshot/target path 的 APPLIED `CREATE_HARDLINK` journal 和 after snapshot 证明当前 ownership；journal 缺失/状态漂移、target 被替换、source inventory/metainfo/binding/ownership tag 漂移都必须返回稳定错误且不得修改 task/journal/downloader。成功路径要在 hash 后再次复核关键证据，并证明所有下载器写方法调用次数为 0。GET 响应脱敏测试必须断言 hash、ownership、journal ID、device/inode、绝对 source path 等 canary 不出现；POST repair action 也必须证明客户端只提交固定动作和 Idempotency-Key，所有安全事实由后端重新取得。
...
health 还必须验证 circuit CLOSED/OPEN/HALF_OPEN、retry-after、限流等待、cache hit/miss/eviction、真实请求与重试计数均与 fake clock/调用次数一致；人工 `reset_circuit` 保留累计指标且不产生虚假的 recovery 通知。health/API 响应不得包含 base URL、凭证或第三方正文。读取 health 需要管理员会话，管理员 reset 需要 CSRF + `If-Match`。熔断打开/恢复通过通用 notification outbox 的 `SITE` subject 聚合，相同站点+事件键复用同一 outbox；非法 error code 必须退化为固定安全码。`0014 → 0015` 迁移必须把已有 task outbox 回填为 `TASK` subject 且保持 task/event 外键证据。
...
- `/api-tokens` 路由不存在，Bearer 请求不能替代管理员会话鉴权。
...
- 通知渠道 GET 只返回 `credential_configured`；前端编辑默认 `KEEP`，只有显式替换凭证才发送新 secret，关闭/成功/失败后清空凭证输入。创建、更新、删除和启停分别验证管理员会话、CSRF 与强 `If-Match`。
...
M6 计划备份门禁验证默认关闭、启用后到期执行/未到期跳过、手动 force 备份、计划与手动运行互斥、retention 复用、运行状态不递增策略 version，以及 API 的管理员会话、CSRF、强 `If-Match`/412 冲突。所有数据库和备份目录均为 pytest 临时目录，不连接 PT/qB/TR 或媒体目录。
...
- 公开 `execute`/`cancel` 必须要求管理会话 CSRF；副作用动作缺少 `Idempotency-Key` 返回 428。键明文不得落库，同 actor/key 同请求只调用一次底层 coordinator，同键不同请求返回 `IDEMPOTENCY_CONFLICT`。可安全归类失败应持久化并重放；未分类异常保持 PENDING。还必须覆盖 receipt PENDING 后 task 已被后台推进到后续 stage 的 execute 重放，确认不会重新进入 LINKING 或倒退状态。
...
- 任务事件接口必须覆盖历史顺序、`after_event_id` 增量、跨 task 游标拒绝、SSE `Last-Event-ID` 续接、no-cache/no-buffering 与脱敏字段边界。前端优先 EventSource，同源管理员会话 cookie 鉴权；SSE 断线才启用事件增量轮询。
