...
公开 repair execute API 还必须覆盖独立 action receipt 边界：admin session 缺 CSRF 必须在 service 前阻断，`tasks:write` Token 可无 CSRF 调用而 `tasks:read` 必须 403；缺失/非法 `Idempotency-Key` 分别稳定返回 428/422，相同 actor+key 的请求只能命中同一 receipt。故障注入至少覆盖 coordinator 已完成 inode isolation 与 repair checkpoint 推进、但 receipt 尚未 SUCCEEDED 时响应丢失，重放必须凭当前服务端 checkpoint 补记同一 receipt 且不新增 isolation。请求体只能有固定 execute 动作，响应/receipt payload 不得出现 torrent hash、ownership、operation journal、repair candidate/evidence digest、device/inode 或绝对路径。
