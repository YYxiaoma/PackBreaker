...
内部修复链现已从 `TaskRepairIsolationCoordinator` 延伸到 downloader repair executor，并通过受控 action API 对外开放。isolation coordinator 每次只从服务端可信 AUTO_PIECE plan 取得一个当前仍需隔离的 HARDLINK，重新证明 downloader owned torrent 仍 stopped、source inventory/current execution plan 未变化，再把原 hardlink journal ID 交给 journal-backed isolation 原语；一项完成后重新生成计划，而不是一次冻结整批 inode 事实。全部必要隔离完成后，`TaskRepairCoordinator` 才把服务端冻结的 repair 证据写入 checkpoint 并将任务从 `RETRY` 推回 `CLIENT_VERIFYING`。

ownership handoff 不改写历史 `CREATE_HARDLINK` journal：原 journal 继续保存“最初由 PackBreaker 建立 hardlink”的不可变证据，新的 APPLIED `ISOLATE_REPAIR_TARGET` journal 明确引用该 hardlink journal 并成为当前 target inode 的后继 ownership 证据。客户端下载写入前，repair-plan 再生成仍要求 isolation after snapshot + source 字节一致；一旦同一 repair cycle 已由 APPLIED repair-start/repair-stop 和独立二次 verify/recheck journal 证明客户端下载确实写过该 target，则后续失败重试不再要求 target 内容等于 source，而改为严格证明 source snapshot 未变、target 仍是 isolation journal 登记的同一 device+inode、`link_count=1`、长度正确且与 source inode 不同。这样允许第二轮修复继续使用合法可变内容，但绝不允许 inode 被外部替换。若 isolation 仍是 `INTENT_RECORDED`，coordinator 会优先恢复唯一的 journal-owned in-flight 操作，`RECONCILE_REQUIRED` 则继续失败关闭、不得自动越过。

补齐阶段使用独立 `QBITTORRENT_REPAIR_START/STOP` 或 `TRANSMISSION_REPAIR_START/STOP` journal，不复用做种 start。repair-start 必须绑定原 APPLIED add、上一轮 APPLIED verify/recheck、当前 checkpoint 和全部 APPLIED isolation；客户端下载达到 100% 后必须先 journal-backed stop，再使用独立 `repair_candidate_key` 发起新的 qB recheck/TR verify。完整才进入既有 `SEEDING`，仍不完整则回 `RETRY` 并形成下一轮 repair ownership 证据。四类 repair start/stop journal 现也进入只读安全对账：只有已经保存可信 after snapshot 的 `RECONCILE_REQUIRED` 记录才允许读取当前 owned torrent，并重新核对 downloader ID/version、hash、save path、ownership 与进度/停止状态；任何未知结果缺少 after snapshot 时仍只能人工检查，自动对账绝不重发 start/stop。

取消链现已完成 isolation ownership handoff 的专用清理闭环。取消 checkpoint 升级为 v3 并继续兼容 v1/v2；若 `rollback_created_resources=true`，必须先由同 execution plan 的 qB/TR remove journal 证明下载器已移除，随后为每个 APPLIED isolation 新建独立 `CLEANUP_REPAIR_TARGET` journal。cleanup 只删除 isolation after snapshot 登记的同 device+inode、同长度、`link_count=1` 普通文件；不要求内容或 mtime 等于隔离时快照，因为修复下载可能已合法改写字节。若路径已被外部替换、额外 hardlink 或身份漂移，目标原样保留且 cleanup 转 `RECONCILE_REQUIRED`。unlink 已完成但 APPLIED 尚未提交时，可凭路径仍不存在恢复同一 cleanup journal；原 `CREATE_HARDLINK` 与 `ISOLATE_REPAIR_TARGET` journal 始终保持 APPLIED 不改历史。若 `rollback_created_resources=false`，独立 target 明确记录为 retained，不自动删除。

任务级可信 repair-plan API 与 execute action 都坚持“浏览器零安全事实输入”。`GET /task-units/{unit_id}/repair-plan` 只接受 `AUTO_PIECE`、`FILE_ONLY`、`GUIDED` 三种 mode；`POST /task-units/{unit_id}/repair/actions` 只接受固定 `{action:"execute"}` 与 `Idempotency-Key`。两者都不接受浏览器提交暂停状态、inode、ownership、hash、journal 或空间证据。POST 受 admin CSRF 或 `tasks:write` Token scope 保护，并由独立 `repair_execute` action receipt 提供请求级幂等与响应丢失恢复。
...
GET 响应只返回相对 torrent path、piece/file 影响范围、是否需要隔离、补齐/空间预算和固定阻断原因；POST 响应只返回 task/unit/status/version/execution-plan、receipt 与 replay 标志。torrent hash、ownership tag、operation journal ID、repair candidate/evidence digest、device/inode、绝对源路径与 operation payload 都留在服务端。GET 不暂停下载器、不修改 task/journal、不调用 add/recheck/start/remove，并固定 `execution_allowed=false`；这表示“该快照不是写权限”，而不是“系统没有受控执行能力”。POST 每次都会重新证明当前证据后才进入真实修复链。
