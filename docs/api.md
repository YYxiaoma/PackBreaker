...
operation journal 保留期清理与资源回滚严格分离。`GET /operations/retention-plan` 只把 `NOOP` / `ROLLED_BACK` 当候选，并要求所属任务已经 `DONE` / `FAILED` / `CANCELLED`，候选 journal 与终态 task 都早于截止时间，而且 task checkpoint、历史 action receipt、同 task 其他 journal 的 JSON 中都没有精确引用候选 journal ID。`purge` 动作在写事务内重新证明同一组条件，再把原 journal 的 payload 删除并写入 `operation_journal_tombstone`；tombstone 只保留 task/journal ID、operation type、最终状态、原时间戳、幂等键摘要与完整 journal digest，不保留 target/intent/snapshot。原 operation 幂等键此后会被 tombstone 永久拒绝，避免清理后重放历史副作用。journal 删除、tombstone 与 purge receipt 成功在同一事务提交，因此 HTTP 响应丢失只需按原 Idempotency-Key 重放。`APPLIED`、`INTENT_RECORDED`、`RECONCILE_REQUIRED`、`ROLLBACK_PENDING`、`ROLLBACK_BLOCKED` 均不能进入 retention purge。
...
| GET | `/system/upgrade` | 查询正式升级目标与 helper 进度；不会把 docker.sock 暴露给主应用 |
| POST | `/system/upgrade/actions` | 启动受幂等保护的一键升级；helper 负责 Docker 切换、健康验证与失败回滚 |


升级动作只接受服务端重新确认仍为最新正式 Release 的 `target_version + target_image_digest`。主 PackBreaker 若自身检测到 `/var/run/docker.sock` 会以 `UPGRADE_MAIN_DOCKER_SOCKET_PRESENT` 拒绝执行；Docker 管理权限必须隔离到独立 `packbreaker-updater`。helper 通过共享 `/config/updater/updater.sock` Unix socket 与随机 token 接收请求，持久化 phase；活动请求重复提交同一 `Idempotency-Key` 返回同一升级身份，不创建第二份备份或第二次容器切换。helper 异常重启、状态损坏或自动回滚无法收敛时进入 `manual_recovery_required`，后续自动升级失败关闭。
