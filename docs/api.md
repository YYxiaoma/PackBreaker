...
| GET | `/system/status` | 版本、任务统计、脱敏依赖状态与后台活动任务 driver 计数/最近 tick 状态 |
...
| POST | `/tasks/{task_id}/operations/{journal_id}/actions` | 当前仅支持 `reconcile`；要求 `tasks:write`、`Idempotency-Key`，管理会话另需 CSRF；不接受任意目标状态 |
