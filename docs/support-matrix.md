...
**新增门禁待验收**：原生 ARM64 Runner 已增加独立无外网 Docker qBittorrent 5.2.3 / Transmission 4.1.3 真实协议测试（仅合成种子和一次性配置），以及 repair inode 隔离、xattr 归属和 journal 崩溃恢复回归。须以新 CI Run 实际结果为准；不接触用户真实下载器或媒体。
