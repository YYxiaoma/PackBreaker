...
当前开发 Runner 没有 Docker daemon，但 GitHub Actions 已持续承担真实容器门禁。当前最新正式 Release 为 `v0.1.8`，Release workflow run `35366864779` 已成功完成正式发布，公开 GHCR 不可变镜像为 `ghcr.io/yyxiaoma/packbreaker@sha256:f114296a40c3bc68f036071382ea3029818fc909ef78a2c07a5ab382e6c4d0d3`。当前源码保持 v0.1.8 版本线；正式容器升级/回滚、Compose labels 保留、browser E2E 与 updater helper 证据已由 main CI / Release workflow 取得。
...
- `release-baseline.json` 当前固定正式 `v0.1.8` digest；后续候选版本必须以该最新 published baseline 做相邻版本升级/回滚门禁后才允许进入正式发布。
