...
**GHCR 多平台子镜像清单门禁（新增，待 CI）**：发布工作流对每个宣称的平台读取不可变 child manifest，核对真实子镜像配置与层描述符存在且结构有效，拒绝无效或重复 digest 等异常；离线测试尚不等于正式 v1.0.0 GHCR 发布后的真实性验证，子 config 实际 CPU 架构、版本标签和原生运行仍需独立检查。
