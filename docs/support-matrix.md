...
**新增同一任务真实客户端下载器门禁（待 CI）**：原生 ARM64 隔离 qBittorrent 测试以合成源文件/模拟站点和模拟管理员审批身份创建真实的任务、审核、执行计划及 Hardlink/ADD/START journal，并在同一任务中经真实客户端收敛到 DONE，含未授权失败关闭、重放与源文件安全检查；不使用生产媒体或凭据。完成该门禁仍不等于真实 PT、Web UI 人工点击、Transmission 同一任务全链路及正式 ARM64 发布/跨版本升级已获验证。
