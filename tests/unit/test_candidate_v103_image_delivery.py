"""Keep v1.0.3 GHCR test image source-pinned and isolated from formal release."""
...
WORKFLOW = ROOT / ".github" / "workflows" / "candidate-v103-image-delivery.yml"
REVIEWED_SHA = "b2d5c69c730eefc01ef430010d7cf8846d39e56c"
BRANCH = "candidate/v1.0.3-docker-socket-upgrade"
...
def test_delivery_pins_reviewed_source_and_cannot_touch_formal_tags() -> None:
    ...
    assert triggers["push"]["paths"] == [".github/workflows/candidate-v103-image-delivery.yml"]
    assert workflow["env"]["CANDIDATE_SHA"] == REVIEWED_SHA
    assert workflow["env"]["CANDIDATE_VERSION"] == "1.0.3"
    ...
    assert "candidate-v1.0.3-" in source
