...
from scripts.validate_release_baseline import project_version
...
def test_readiness_failure_does_not_disclose_container_bootstrap_logs(tmp_path: Path) -> None:
    ...
    fake_bin.mkdir()
    # After a release, project_version == formal baseline version. Model the
    # next *unreleased* candidate here so the isolated readiness/privacy path
    # is reached without weakening the production stale-candidate rejection.
