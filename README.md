...
项目当前已从交互原型阶段进入 M1 安全骨架研发：仓库包含可运行的前端交互原型，以及后端领域安全规则、启动配置、单实例执行锁、FastAPI 存活/就绪检查、SQLite WAL / SQLAlchemy 持久化、Alembic 迁移、任务/操作日志 repository、管理员首次初始化与持久会话/CSRF、API Token scope/过期/撤销、可信代理与安全响应头、AES-256-GCM secret store，以及下载器配置 CRUD、qBittorrent/Transmission 只读连接与能力探测、路径映射/硬链接可行性诊断。当前下载器适配器不暴露添加、删除、暂停、恢复、校验等任务写操作；真实站点接入、生产文件创建/修复与辅种执行仍未实现。
